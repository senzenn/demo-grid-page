...
const homeNavs: NavItem[] = siteConfig.nav.links;
...
export function NavMenu() {
  ...
  const navs = React.useMemo(() => {
    if (isHome) {
      ...
      // Combine all dashboard navigation items
      return [
        ...navigationData.mainNavItems,
        ...navigationData.paymentsNavItems,
        ...navigationData.toolsNavItems,
        navigationData.settingsItem,
      ].map(item => ({
        name: item.title,
        href: item.url
      }))
  ...
  React.useEffect(() => {
    ...
    const href = isHome ? `#${navs[0].href.substring(1)}` : navs[0].href
    const firstItem = ref.current?.querySelector(`[href="${href}"]`)?.parentElement;
    if (firstItem) {
  ...
  }, [isManualScroll, isHome, navs]);

  const handleClick = (
    e: React.MouseEvent<HTMLAnchorElement>,
    item: NavItem,
  ) => {
    // For dashboard pages, don't prevent default - let Next.js handle navigation
